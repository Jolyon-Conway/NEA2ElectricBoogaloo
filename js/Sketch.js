...
function draw() {
    ...
    for (let i = 0; i < faces.length; i++) {
        ...
        for (let j = 0; j < faces.length - 1; j++) {
            if (Math.sqrt(Math.pow(faces[j].findCentre().x, 2)) < Math.sqrt(Math.pow(faces[j + 1].findCentre().x, 2))) {
                let temp = faces[j];
                faces[j] = faces[j + 1];
                faces[j + 1] = temp;
            }
            if (Math.sqrt(Math.pow(faces[j].findCentre().y, 2)) < Math.sqrt(Math.pow(faces[j + 1].findCentre().y, 2))) {
                let temp = faces[j];
                faces[j] = faces[j + 1];
                faces[j + 1] = temp;
            }
            if (faces[j].findCentre().z > faces[j + 1].findCentre().z) {
                let temp = faces[j];
                faces[j] = faces[j + 1];
                faces[j + 1] = temp;
            }
